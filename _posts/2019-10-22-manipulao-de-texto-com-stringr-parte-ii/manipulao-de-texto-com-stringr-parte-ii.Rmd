...
[No post anterior](https://www.fulljoin.com.br/posts/2019-10-13-manipulao-de-texto-com-stringr/) sobre manipulação de texto, exploramos três funções do pacote `stringr`: `str_pad`, `str_sub` e `str_detect`. 

Neste post, continuaremos utilizando o `str_detect`, mas, dessa vez, envolveremos noções de **regex**. Além dessa função, também veremos outras funções úteis para a manipulação de texto. Da mesma forma que no post anterior, também iremos criar problemas e resolvê-los. 
...
O objeto `paises` possui três colunas cujos elementos apresentam pelo menos uma letras maiúscula.  Se quiséssemos que todos os caracteres tivessem letras minúsculas poderíamos usar a função `str_to_lower`. No primeiro argumento da função passamos o vetor de textos cujas letras queremos que sejam todas minúsculas. Vejamos os 10 primeiros elementos da coluna "ISOA" do objeto `paises`:
...
Agora modifiquemos as três colunas do objeto `paises` para que seus elementos apenas apresentem letras minúsculas. Compare com a primeira tabela que apresentamos no início deste post.
...
Pode-se notar que a coluna "pais_ing" voltou a apresentar palavras cuja primeira letra foi capitalizada. Compare a coluna "pais_ing" do resultado de `head(paises)` do problema n. 1 com o resultado de `head(paises)` do problema n. 2.
...
Acima observamos que mesmo tendo inserido o padrão "can" (tudo minúsculo) foi possível encontrar a mesma sequência de letras, ainda que elas fossem todas maiúsculas. Perceba que se `ignore_case = FALSE`, a padrão "can" não detectaria o padrão textual "CAN" na coluna "ISOA".
