...
**Como objetivo a ser alcançado nesse post, gostaríamos de ter um panorama geral das empresas listadas na bolsa de valores brasileira e entender um pouco mais sobre esse cenário para tentar tirar alguns insights.**
...
Vamos manipular um pouco para deixar o dataframe perfeito e em seguida trabalhar alguns gráficos. Primeiro vamos separar os setores. Aparentemente cada empresa tem 3 setores em diferentes níveis de detalhes.

```{r}
dados <- dados %>% 
  separate(setores, c("setor.1", "setor.2", "setor.3"), sep = '/') %>%
  mutate_at(.vars = vars(starts_with('setor')), .funs = ~trimws(.))

unique(dados$setor.1)
unique(dados$setor.2)
unique(dados$setor.3)
```
...
```{r, eval=FALSE}
write_csv(dados, 'empresas_b3_2019-03-31.csv')
```

## Alcançando o objetivo

Construída a base de dados, vamos nos informar para conhecer o cenário atual das ações listadas na B3. Lembrando que pegamos apenas os dados reportados pelas empresas referentes ao período de 01/01/2019 a 31/03/2019.
...
No treemap acima, somamos o total do patrimônio para avaliarmos as proporções, mas é curioso notar que existem empresas com patrimônio líquido negativo. Vamos verificar os grandes setores separando o total de patrimônio líquido negativo e positivo.
...
dados %>% 
  ...
  group_by(setor.1, patrimonio) %>% 
  summarise(patrimonio.total = sum(patrimonio.liquido)) %>% 
  ungroup() %>% 
  ggplot(aes(x = reorder(setor.1, patrimonio.total), y = patrimonio.total, fill = patrimonio))+
  geom_bar(stat = "identity")+
  coord_flip() + 
  theme(legend.position = "none") +
  labs(title = "Patrimônio positivo e negativo por grandes setores",
       y = "Patrimônio",
       x = "Grandes Setores")

```

Comportamento parecido existe no somatório do lucro líquido dos grandes setores. Faremos o mesmo exercício separando o lucro líquido positivo e o lucro líquido negativo (entendo que seja prejuízo!)

```{r}
dados %>% 
  mutate(lucro = ifelse(lucro.liquido > 0, 'positivo', 'negativo')) %>% 
  group_by(setor.1, lucro) %>% 
  summarise(lucro.total = sum(lucro.liquido)) %>% 
  ungroup() %>% 
  ggplot(aes(x = reorder(setor.1, lucro.total), y = lucro.total, fill = lucro))+
  geom_bar(stat = "identity") +
  coord_flip() + 
  theme(legend.position = "none") + 
  labs(title = "Lucro e prejuízo por grandes setores",
       y = "Lucro",
       x = "Grandes Setores")
```
...
```{r}
dados.lucro.acao <- dados %>% 
  group_by(setor.1, setor.2, setor.3, nome.empresa) %>% 
  summarise(lucro.total = sum(lucro.liquido),
            total.acoes = sum(total.acoes)) %>% 
  filter(total.acoes > 0) %>% 
  mutate(lucro.acao = round(lucro.total/total.acoes, 2)) %>% 
  ungroup()

# View(dados.lucro.acao)
library(ggeconodist)


ggplot(dados.lucro.acao, aes(x = setor.1, y = lucro.acao)) +
  geom_econodist(median_point_size = 1.2,
    tenth_col = "#b07aa1",
