...
Agora vamos cruzar os dados com a carteira e calcular os rendimentos.
...
Depois de apanhar um pouco para entender a melhor forma de calcular o retorno acumulado, de forma agregada e desagregada, e depois de ótimas dicas de um amigo, entendi que o ideal é tratar sua carteira como um único fundo de cotas, e suas compras e venda, independente da ação, como compras de cotas desse fundo. Dessa forma, caso queira desagregar para ver o rendimento em diferentes níveis, basta tratar a agregação desejada como um fundo a parte. 

Sumarizamos por dia e pegaremos a quantidade de ações custodiadas vezes seu respectivo preço de fechamento divido pelo valor total aplicado. Dessa forma teremos o valor da cota desse dia. Façamos o mesmo para o preço ajustado, que difere do preço de fechamento pelos ajustes feitos (direto na fonte) para descontar dividendos e outras coisas. Dividido a cota com valor ajustado pela cota de fechameto temos uma cota real.

Sendo assim, escolhemos o primeiro dia e calculamos a cota inicial. A partir dela vamos equalizando a compra e venda de novas cotas:
...
Com o valor da cota inicial e com o valor de mercado de cada investimento calculamos o retorno dia a dia com base na cota inicial e reequilibrando o valor da cota com os novos aportes e com os preços de mercado já calculados.
...
Uma simples função para calcular o retorno em um perído agregado
...
Pronto, agora baixamos a série do índice Bovespa para um benchmark mínimo e aplicamos o mesmo cálculo de retorno/cota. Fazemos um join com o benchmark onde a data é a chave e teremos um data frame com ambas as séries de retornos acumulados prontas para comparação.
...
Com um simples gráfico utilizando o plotly + ggplot2 podemos visualizar o comportamento do retorno acumulado comparado com nosso benchmark. Percebe-se que minha carteira (sem os fundos imobiliários!) tem um desempenho lastimável frente ao principal índice de comparação IBOV, revelando minha falta de talento para comprar ações.
...
Com poucas adaptações conseguiríamos calcular rendimentos mensais acumualdos e inserir mais séries como benchmarnk.

## Conclusão

Além de ter excluído alguns tickers por problemas na API que usamos como fonte, há uma série de ajustes que precisam ser feitos na carteira para calcular com mais precisão: tratar desdobramentos, impostos, corretagens, etc.

Além de precisar de uma API mais confiável do que a do Yahoo, seria legal também adaptar a leitura dos dados para flexibilizar um possível input via planilha de Excel, assim possibilitaria trazer para o R controles de compra e venda já existentes feitos fora do CEI.

Em um próximo post vamos tentar explorar mais a fundo a composição da carteira, correlações e alguma avaliação simplificada de riscos.